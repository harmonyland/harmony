...
export const channelUpdate: GatewayEventHandler = async (
  ...
  const oldChannel: Channel | undefined = await gateway.client.channels.get(
    d.id,
  );
  ...
  if (oldChannel !== undefined) {
    ...
    if ("guild_id" in d) {
      ...
      guild = await gateway.client.guilds.get(
        (d as GuildChannelPayload).guild_id,
      ) as Guild | undefined;
    }
    if (oldChannel.type !== d.type) {
      const channel: Channel = getChannelByType(gateway.client, d, guild) ??
        oldChannel;
