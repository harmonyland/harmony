...
export class Interaction extends SnowflakeBase {
  ...
  async editResponse(
    ...
    await this.client.rest.patch(url, {
      content: data.content ?? "",
      embeds: data.embeds ?? [],
      flags: typeof data.flags === "object"
  ...
  get url(): string {
    return `https://discord.com/api/v${this.client?.rest?.version ??
      Constants
        .DISCORD_API_VERSION}/webhooks/${this.applicationID}/${this.token}`;
  }
  ...
  async send(
    ...
    const payload: any = {
      content: text,
      embeds: (option as WebhookMessageOptions)?.embed !== undefined
      ...
      components: (option as WebhookMessageOptions)?.components === undefined
        ? undefined
